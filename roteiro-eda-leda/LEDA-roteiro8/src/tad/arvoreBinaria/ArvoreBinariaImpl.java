...
public class ArvoreBinariaImpl<T extends Comparable<T>> implements ArvoreBinaria_IF<T>{
	...
	private NoArvoreBinaria<T> buscar(T chave, NoArvoreBinaria<T> root)
		...
		if(root.compareTo(new NoArvoreBinaria<T>(chave)) == 0)
			return root;
		if(root.compareTo(new NoArvoreBinaria<T>(chave)) > 0) // Esquerdo
	...
	public void inserir(T chave) throws ExceptionBST {
		...
		else if(this.raiz.compareTo(new NoArvoreBinaria<T>(chave)) > 0) { // Esquerdo
		...
		}else if(this.raiz.compareTo(new NoArvoreBinaria<T>(chave)) < 0){ // Direito
	...
	private void inserir(T chave, NoArvoreBinaria<T> root) throws ExceptionBST {
		...
		}else if(root.compareTo(new NoArvoreBinaria<T>(chave)) < 0){ // Direito
	...
	public NoArvoreBinaria<T> remover(T chave) throws ExceptionBST, ArvoreVaziaException {
		...
		return new NoArvoreBinaria<T>(chave);
	}
	...
	private NoArvoreBinaria<T> minimo(NoArvoreBinaria<T> node) {
	...
	private NoArvoreBinaria<T> maximo(NoArvoreBinaria<T> root2) {
